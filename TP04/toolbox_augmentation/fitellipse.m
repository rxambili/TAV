function [a,CC,CCInv] = fitellipse(X,Y)
...
% Modified by P. Gurdjos ----------------------------------------------------------
% par = [a;b;c;d;e;f] with  x^2*a + x*y*b + y^2*c + x*d + y*e+f = 0
% CC : ellipse homogeneous matrix =
% [     a, 1/2*b, 1/2*d]
% [ 1/2*b,     c, 1/2*e]
% [ 1/2*d, 1/2*e,     f]
CC = [ par(1),par(2)/2,par(4)/2;par(2)/2,par(3),par(5)/2;par(4)/2,par(5)/2,par(6) ];
if nargout == 3
   CCInv = inv(CC);
end
% ---------------------------------------------------------------------------------- 
