...
for canal = 1:N
	...
	line([x1,x1],[y1,y2],'Color',couleur_classes(canal,:),'Linewidth',2);
	line([x1,x2],[y2,y2],'Color',couleur_classes(canal,:),'Linewidth',2);
	line([x2,x2],[y2,y1],'Color',couleur_classes(canal,:),'Linewidth',2);
	line([x2,x1],[y1,y1],'Color',couleur_classes(canal,:),'Linewidth',2);
	drawnow;
	...
	moyennes(:,canal) = moyenne;
	nb_donnees = size(echantillons,2);
	echantillons_centres = echantillons-moyenne*ones(1,nb_donnees);
	variances_covariances(:,:,canal) = echantillons_centres*transpose(echantillons_centres)/nb_donnees;
end
